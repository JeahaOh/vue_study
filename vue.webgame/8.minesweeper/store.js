  ...
  mutations: {
    ...
    [OPEN_CELL](state, { row, cell }) {
      ...
      function checkAround(row, cell) {
        console.group("checkAround");

        const checkIsAroundUndefined =
          row < 0 || row >= state.tableData.length || cell < 0 || cell >= state.tableData[0].length;
        if (checkIsAroundUndefined) {
          return;
        }
        ...
        console.log("checkAround", counted.length);
        console.groupEnd("checkAround");
      }
      ...
      if (state.data.row * state.data.cell - state.data.mine === state.opendCount + opendCount) {
        halted = true;
        result = `END WITH ${state.timer}' sec`;
      }
